...
class GetInstructorReportPDFView(APIView):
    renderer_classes = [ReportsRenderer]
    permission_classes = [IsAuthenticated, 
                          RoleOrPermissionCheck.for_permission_or_roles(
                              "instructor__pdf_report",
                            [SuperAdmin]
                        )]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    search_fields = ['first_name',"last_name","email","is_active","created_at","category"]
    ...
    def get(self, request, format=None):
        ...
        topics = [user for user in topics if has_role(user, Instructor)]
        ...
        template = get_template('pdf/instructor_report.html')
        html  = template.render(data)
        with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as temp_file:
            pdf_path = temp_file.name
...
class GetInstructorReportExcelView(APIView):
    renderer_classes = [ReportsRenderer]
    permission_classes = [IsAuthenticated, 
                          RoleOrPermissionCheck.for_permission_or_roles(
                              "instructor_excel_report",
                            [SuperAdmin]
                        )]
    pagination_class = CustomPageNumberPagination
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
...
class GetCorporateStudentsActivityLogPDFReportView(APIView):
    ...
    permission_classes = [IsAuthenticated, 
                            RoleOrPermissionCheck.for_permission_or_roles(
                                "export_student_activity_pdf_report",
                            [SuperAdmin]
                        )]
    def get(self, request, id=None):
...
class GetPartnerRequestListingView(APIView):
    ...
    def get(self, request, format=None):
        ...
        plans = PartnerRequests.objects.all()
        ...
        paginator = self.pagination_class()
        page = paginator.paginate_queryset(plans, request, view=self)
        serializer = PartnerRequestsListSerializer(page, many=True)
        return paginator.get_paginated_response(serializer.data)
    


class PDFPartnerRequestReportView(APIView):
    renderer_classes = [ReportsRenderer]
    permission_classes = [IsAuthenticated, 
                          RoleOrPermissionCheck.for_permission_or_roles(
                              "job_application_pdf_report",
...
class CSVPartnerRequestReportView(APIView):
    renderer_classes = [ReportsRenderer]
    permission_classes = [IsAuthenticated, 
                          RoleOrPermissionCheck.for_permission_or_roles(
                              "job_application_excel_report",
