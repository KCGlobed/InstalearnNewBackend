...
class CancelSubscriptionView(APIView):
    renderer_classes = [SubscriptionRenderer]
    def post(self, request, format=None):
        serializer = CancelSubscriptionSerializer(data = request.data, context={'user':request.user})
        if serializer.is_valid(raise_exception = True):
            serializer.save()
            return success_response(message="Subscription cancelled successfully!", data={}, status_code=status.HTTP_200_OK)
...
class UpgradeSubscriptionView(APIView):
    renderer_classes = [SubscriptionRenderer]
    def post(self, request, format=None):
        serializer = UpgradeSubscriptionSerializer(data = request.data, context={'user':request.user})
        if serializer.is_valid(raise_exception = True):
            serializer.save()
            return success_response(message="Subscription upgraded successfully!", data={}, status_code=status.HTTP_200_OK)
